...
st_bbox(counties_sf) #creates a bounding box around the feature
...
huc8_sf_UppnerNeuse <- st_read('./Data/Spatial/NCHUC8.shp') %>% 
  filter(SUBBASIN == 'Upper Neuse')

#Create a map
mapview(huc8_sf, col.regions = 'orange')+
  mapview(huc8_sf_UppnerNeuse, col.regions = 'purple')
...
svi2018_nc_raw <- read.csv(
  'https://svi.cdc.gov/Documents/Data/2018_SVI_Data/CSV/States_Counties/NorthCarolina_COUNTY.csv', #this is a tabular dataset
  colClasses = c('FIPS' = 'factor')) %>%  #this specifies FIPS column needs to be imported as a factor (this is important because we do not want to lose leading zero)
  select(COUNTY, FIPS, LOCATION, E_TOTPOP, E_POV, E_MINRTY)

#Check structure
str(svi2018_nc_raw)

#Join the SVI attributes to the county spatial features - this is the base r version of a join
...
huc8 <- read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv',
                  colClasses = c('HUC_12'='factor')) %>% 
  mutate(HUC_8 = substr(HUC_12,1,8)) %>% #substr allows you to take the first 8 values of the value
  group_by(HUC_8) %>% 
...
huc8_sf_wind <- huc8_sf %>% 
  left_join(huc8, by = c('HUC_8' = 'HUC_8'))
...
state_sf <- counties_sf %>% 
  group_by('STATEFP') %>% 
  summarize(ALAND = sum(ALAND)) #calculating total land area of NC
...
neuse_intersect_2 <- counties_sf_utm %>% 
  filter(st_intersects(x = ., y = neuse_sf, sparse = FALSE)) #y is feature you want to use to select the counties

#View the result - spatial overlay
...
leaflet() %>% addTiles() %>% 
  ...
  addPolygons(data=huc8_sf,weight=1) # leaflet will act weird with UTM data
```
