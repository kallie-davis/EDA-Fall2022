...
#Set the URL
theURL <- 'https://waterdata.usgs.gov/nwis/uv?search_site_no=02087500&period=7&format=rdb' #we can set the period to "20" in the URL to get a period of 20 days

#Get the data, which starts on line 30
gage_data <- read.table(theURL,skip = 29,header=TRUE,sep='\t',stringsAsFactors = T) #used for reading tab separated files
#there are 29 rows before we get to the data so we want to skip 29
...
theURL.2 <- 'https://waterdata.usgs.gov/nwis/uv?search_site_no=02085070&period=24&format=rdb'

gage_data.2 <- read.table(theURL.2,skip = 29,header=TRUE,sep='\t',stringsAsFactors = T)

colnames(gage_data.2) = c("agency_cd", "site_no", "datetime", "tz_cd",
                        "discharge_cfs", "89192_00060_cd", "gage_ht_ft", "89193_00065_cd")

gage_data.2 <- gage_data.2 %>% 
  select(datetime, discharge_cfs,gage_ht_ft) %>% 
  mutate(datetime = ymd_hm(datetime))

#Plot
ggplot(gage_data.2,aes(x=datetime,y=discharge_cfs)) + 
  geom_line() +
  labs(x = "Date",y = "Discharge (cfs)")
```
...
pcode = '00060' #discharge (cfs) #pcode is parameter code

#Identify statistic code for daily values: 
scode = "00003"  #mean #scode is stat code
...
neuse <- renameNWISColumns(neuse); #function fixes the column names
colnames(neuse)
...
nc_pop <- 
  get_acs(geography = "county", #at what level do you want the data
          variables = "B01003_001", #look through documentation for variables (there may be an API for this?)
          state = "NC",
          geometry = TRUE,
          key = "d4ad7874c996c7a83fec0a81466309ddc07d8e22") #a spatial dataframe
...
scrape.it <- function(the_year){
  ...
  the_url <- ifelse(the_year==2021,
                    'https://www.eia.gov/electricity/state/',
                    paste0('https://www.eia.gov/electricity/state/archive/',the_year,'/'))#if not this is the url I want
