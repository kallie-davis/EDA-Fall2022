...
```{r}
ggplot(EnoDischarge, aes(x = datetime, y = discharge.mean)) + #First step is to plot variable over time; in this case we see a large data gap
  geom_line() +
...
EnoDischarge.early <- EnoDischarge.complete %>%  #we are splitting the dataset into early and late data to perform separate analyses
...
```{r}
EnoDischarge.late_ts <- ts(EnoDischarge.late$discharge.mean, start = c(1985,1), frequency = 365) #transform data into time series; you have to do this to use the stl function
#365 means we are working with daily data
head(EnoDischarge.late_ts)
```
...
wind_data_clean <- 
  wind_data %>% 
  mutate( AWND.clean = zoo::na.approx(AWND) ) #create column with no NA values 
...
ggplot(wind_data_clean ) +
  ...
  ylab("Average Wind Speed") #dot for the one NA is connected by the red line
# never use NA omit or drop NA for data in the middle of the dataset; you need to use NA approx to replace
```
...
f_month <- month(first(wind_data_clean$DATE)) #define first month
f_year <- year(first(wind_data_clean$DATE)) #define first year
wind_data_ts <- ts(wind_data_clean$AWND.clean,
                   start=c(f_year,f_month), #we make the first month and year automatic
                   frequency=12) 

#decompose
wind_data_decomp <- stl(wind_data_ts,s.window = "periodic")
plot(wind_data_decomp) #now we know we are dealing with seasonal data

# Run SMK test
wind_data_trend1 <- Kendall::SeasonalMannKendall(wind_data_ts) #null hypothesis is that the data is stationary
