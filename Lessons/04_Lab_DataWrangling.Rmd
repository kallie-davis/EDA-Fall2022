...
knitr::opts_chunk$set(tidy.opts=list(width.cutoff=80), tidy=TRUE) # implement to wrap code cells so text does not run off page!!! We need formatR to do this. be sure to comment out install packages so knitting does not freak out.
...
> Answer: The row numbers can change. 
...
NTL.phys.data.1999_July_summarize <-
  NTL.phys.data.1999_July %>% 
    filter(depth == 0) %>%
    filter(!is.na(temperature_C)) %>%
    summarise(mean.temp = mean(temperature_C))
...
NTL.phys.data_spread <-
  pivot_wider(NTL.phys.data, 
              names_from = "depth",
              valued_from = "temperature_C")
```
