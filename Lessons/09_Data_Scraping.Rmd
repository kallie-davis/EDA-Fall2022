...
you can type /robots.txt at the end of a url to see if a site has one. Can disallow you to scrape certain data, so this means they do not want you to scrape the data.
...
Hit ctrl U to open the data source. A webpage is just an html which instructs what to show and how to show it which is interpreted by your browser software. 
"< >" indicate a tag (e.g., <head> this is a head tag)
...
the_facility_name <- webpage %>% 
  html_nodes("tr:nth-child(2) th+ .left:nth-child(2)") %>% #Unique node ID for the facility name tr:nth-child(2) th+ .left:nth-child(2)
  html_text()
the_facility_name
...
avg_withdrawals <- webpage %>% 
  ...
  html_text()
avg_withdrawals #always pulls values as characters, so this shows up as a character vector

max_withdrawals <- webpage %>% 
  html_nodes('.table:nth-child(7) td:nth-child(8) , .table:nth-child(7) td:nth-child(4)') %>%
  html_text()  #.table:nth-child(7) td:nth-child(8) , .table:nth-child(7) td:nth-child(4)
max_withdrawals
```
...
df_withdrawals <- data.frame("Month" = rep(1:12), #rep command creates a sequence of numbers one through 12
                             "Year" = rep(2000,12), #make a vector of 12 repeats of the number 2000
                             "Avg_Withdrawals_mgd" = as.numeric(avg_withdrawals), #convert to numeric
...
df_withdrawals <- df_withdrawals %>% 
  mutate(Registrant = !!the_registrant, #!! (bang bang) indicates reference to a variable, do not refer to a string
...
the_year <- 2015
the_scrape_url <- paste0(the_base_url, '/', the_facility, '/', the_year) #paste0 eliminates space when adding to a string
...
the_dfs <- lapply(X = the_years,
                  FUN = scrape.it, #repeat "scrape.it" function for each value of x with the specific facility ID
                  the_facility=my_facility) #this variable is satic in this case

#OR, use purrr's map function
the_dfs <- map(the_years,scrape.it,the_facility=my_facility) #does same thing as lapply; uses map function instead

#Conflate the returned dataframes into a single dataframe
the_df <- bind_rows(the_dfs) #tidyverse version of rbind
...
the_facility_ids <- the_main_website %>% 
  ...
  html_attr("href") %>%           #Pull the "href" attribute from the node; changed from html_text which will just show the text not the attribute
...
the_facility_ids_subset <-  sample(the_facility_ids,20) #subset 20

#Create a list of the year we want, the same length as the vector above
the_years <- rep.int(2020,length(the_facility_ids_subset)) #create a list of years that match the length of facility IDs

#"Map" the "scrape.it" function to retrieve data for all these
dfs_2020 <- map2(the_years, the_facility_ids_subset, scrape.it) #map2 iterates through lists in parallel
...
the_df <- cross2(the_years,the_facilities) %>% #cross2 generates a list of lists
  map(lift(scrape.it)) %>% 
  bind_rows()
