...
>  depth explains 30% of the irradiance. There is a significant negitive correlation between irradiance and depth
...
4. **Residuals vs. Leverage.** This graph will display potential outliers. The values that fall outside the dashed red lines (Cook's distance) are outliers for the model. Watch for drastic departures of the solid red line from horizontal - this is a sign of a poor model fit.
      The problem is when we have both high leverage and high standard residuals (values outside of cook's distance)
...
irradiancebydepth <- 
  ...
  ylim(0, 2000) + #changing the plotting window just to show values between zero and 2000
  geom_point() 
...
```{r, fig.height = 3, fig.width = 4}
PeterPaul.chem.nutrients <- filter(PeterPaul.chem.nutrients,  # need to eliminate zeros to log transform data (we also removed the outlier here)
                                   irradianceWater != 0 & irradianceWater < 5000)
irradiance.regression2 <- lm(data = PeterPaul.chem.nutrients, log(irradianceWater) ~ depth)
summary(irradiance.regression2) # now our R2 value is 0.72
...
irradiancebydepth2 <- 
    ...
    geom_smooth(method = 'lm', se = FALSE, color = "black") #se = FALSE removes confidence interval
print(irradiancebydepth2)
...
We will have more than one x trying to explain the variable y

``` {r, fig.height = 3, fig.width = 4}
TPregression <- lm(data = subset(PeterPaul.chem.nutrients, lakename == "Paul Lake"), #subset to only have observations for Paul Lake
...
par(mfrow = c(2,2), mar=c(4,4,4,4))
plot(TPregression)
par(mfrow = c(1,1)) # not a good residual series as shown in the residual vs fitted plot (there is some other explanatory variable missing)
...
corrplot.mixed(PeterPaulCorr, upper = "ellipse") # printing the upper diagonal only (symmetrical; now you have the value of the correlations shown with the elipses)
```
...
The smaller the AIC the better the model. This even applies to negative values (-250 would be better than -200)

May better represent the data and have a lower R2. One metric will never perfectly represent the data. 
...
step(TPAIC) #our best regression is considering oxygen, temperature, and nitrogen
