...
# Lists ---- you can store whatever you want in lists; you can retreive items by position or by name
#double brackets is the syntax for getting the position of your item
list1 <- list(name = "Maria", mynumbers = vector1, mymatrix = matrix1, age = 5.3); list1
list1[[2]]

# Data Frames ---- every column is a vector; all lists have to have the same number of elements (NA is a placeholder)
...
names(dataframe1) <- c("ID","Color","Passed"); View(dataframe1) # creating new column names

dataframe1[1:2,] # row position (rows 1 through 2)
dataframe1[c("ID","Passed")] # 
dataframe1$ID

```

ctrl alt i to add new code chunk
...
```{r DataExercise-1a}
vector_high <- c(93,93,73,78,85,82,78,75,73,71)
vector_low <- c(65,53,48,59,61,54,51,51,52,55)
...
```{r DataExercise-1b}
celcius_conversion_high <- (vector_high - 32) * 5/9 
celcius_conversion_low <- (vector_low - 32) * 5/9 
#lapply allows you to conduct sd on dataframe (lapply(df,sd))
...
fix.early.dates <- function(d, cutoff) {
       m <- year(d) %% 100  #operator %% is a modular division i.e. integer-divide year(d) by 100 and returns the remainder; m gives the two digit year
