...
#install.packages('sf')
library(sf)                 #we are looking at vector data
...
GasPlants <- st_read('https://opendata.arcgis.com/datasets/a0722062315143a7980cebb79a204781_4.geojson') #st_read can read GeoJSON format off the web
#Alternative
#GasPlants <- st_read('./Data/Spatial/Manufactured_Gas_Plant_Sites.geojson')

#Explore
class(GasPlants) # is a dataframe but is also an sf object
names(GasPlants)
...
EPAair.sites <- EPAair %>% 
  ...
  summarize(
    mean_Ozone = mean(Ozone, na.rm=T),    #What does "na.rm=T" do? Is it necessary? This excludes NA values when calculating descriptive stats in R
...
EPAair.sf <- EPAair.sites %>% 
  st_as_sf(coords = c('SITE_LONGITUDE','SITE_LATITUDE'), #creating a geometry feature; longitude is x, lat is y; we specify crs
           crs=4326)
...
sfg_GraingerHall <- st_point(c(-78.9427,36.00465)) #this is a simple feature coordinate with no reference where it should go
class(sfg_GraingerHall)
mapview(sfg_GraingerHall)

#Upgrade to a simple feature geometry column by adding a crs 
sfc_GraingerHall <- st_sfc(sfg_GraingerHall,crs=4326) #this is a simple feature column
class(sfc_GraingerHall)
mapview(sfc_GraingerHall)

#Upgrade to a spatial dataframe
sf_GraingerHall <- st_as_sf(sfc_GraingerHall) #simple feature table or dataframe by adding attributes
...
the_buffer <- sf_GraingerHall %>% st_buffer(1) #buffer by one degree lat and long - looks odd because we are buffering in spherical coordinates
...
- Notes: Universal transverse Mercator (projected coordinate system) divided into zones. NC is in zone 17. The origin of the coordinate system falls pretty close to NC. Distortion will be lessened closer to the origin. Distance units are in meters.
...
EPSG.io to look up coord ref systems
