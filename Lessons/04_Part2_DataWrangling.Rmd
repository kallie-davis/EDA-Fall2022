...
NTL.nutrient.data.PeterPaul <- droplevels(NTL.nutrient.data.PeterPaul)    # use droplevels
summary(NTL.nutrient.data.PeterPaul$lakename)
...
NTL.nutrient.data.PeterPaul.gathered <- pivot_longer(NTL.nutrient.data.PeterPaul.subset,tn_ug:po4, names_to = "nutrient", values_to = "concentration") # add column for category and one for value

NTL.nutrient.data.PeterPaul.gathered <- subset(NTL.nutrient.data.PeterPaul.gathered, !is.na(concentration))
count(NTL.nutrient.data.PeterPaul.gathered, nutrient) #removing NA values for concentrations; count will return how many rows (counting by nutrients in this example)
