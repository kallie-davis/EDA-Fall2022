...
```{r}
class(NTL.phys.data$lakeid) # can only compare numeric with numeric; character with character; etc. when filtering rows 
...
NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake"))       # %in%
...
NTL.phys.data.1990_1999 <- filter(NTL.phys.data, year4 %in% c(1990:1999))
...
```{r}
NTL.phys.data.depth.ascending <- arrange(NTL.phys.data, depth)         # ascending is default
...
head(NTL.phys.data.temp.descending)
...
interval(first(NTL.phys.data.PeterPaul1$sampledate), last(NTL.phys.data.PeterPaul1$sampledate))  # this is the preferred way!
```
...
Try to separate wrangling from data analysis for a more simple and easy to follow R script.
