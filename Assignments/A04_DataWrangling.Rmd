...
#2
dim(rainfall.data)
colnames(rainfall.data)
str(rainfall.data)

dim(temperature.data)
colnames(temperature.data)
...
```{r}
#3 
#change flow.data date
flow.data$datetime <- as.Date(flow.data$datetime, format = "%m/%d/%y")

flow.data$datetime <- format(flow.data$datetime, "%y%m%d")
create.early.dates <- (function(d) {
       paste0(ifelse(d > 100126,"19","20"),d)
       })
flow.data$datetime <- create.early.dates(flow.data$datetime)
flow.data$datetime <- as.Date(flow.data$datetime, format ="%Y%m%d")

flow.data$datetime <- as.Date(flow.data$datetime, format = "%Y/%m/%d")



#change wind.speed date
windspeed.data$DATE <- as.Date(windspeed.data$DATE, format = "%YYYY-%mm")
