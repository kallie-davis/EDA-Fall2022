...
> Answer:
H0: Lake mean temperature in July does not change with depth across all lakes.
Ha: Lake mean temperature in July does change with depth across all lakes.
...
NTL_LTER_Lake_ChemistryPhysics$month <- as.factor(NTL_LTER_Lake_ChemistryPhysics$month)
...
#5
NTL_LTER_temp_depth <- ggplot(NTL_LTER_Lake_ChemistryPhysics_wrangled, aes(x = depth, y = temperature_C))+
  geom_point(color = "blue")+
  ylim(0, 35)+
  geom_smooth(method = lm, color="black")+
  labs(title = "Temperature by Depth", x = "Depth (m)", y = "Temperature (°C)")
...
> Answer: The figure, if considering the line of best fit alone, suggests a negative association between the two variables; however, if you also consider the distribution of points, it appears that this trend is nonlinear. Overall, as depth increases, the temperature appears to decrease. 73.9% of variation in temperature is explained by depth. 
...
> Answer: As depth increases by one meter there is an associated decrease in temperature of 1.95 degrees celsius. The F-statistic is 2.75e4 with a degrees of freedom of 9726. The p-value is 2.2e-16 which is well below the significance value of 0.001, suggesting that there is a highly significant relationship between temperature and depth and we can reject the null hypothesis. 
...
```{r temperature.model}
#9
temp_AIC <- lm(data = NTL_LTER_Lake_ChemistryPhysics_wrangled, temperature_C ~ year4 + daynum + depth)

step(temp_AIC)

#10
multiple_linear_temp <- lm(data = NTL_LTER_Lake_ChemistryPhysics_wrangled, temperature_C ~ daynum + depth)
...
> Answer: AIC values increase when removing year4, daynum, and depth. This increase in AIC suggests that the removal of these variables made the model worse. In this case the removal of year resulted in a very small increase in AIC so I did not include it in the model. The magnitude of increase in AIC value when considering daynum and especially depth would make the year have very little effect overall in the model. This model explains a variance of 74.1% which is an improvement on the previous model which only uses depth to explain change in temperature. 
...
```{r scatterplot.2}
#14.
ggplot(NTL_LTER_Lake_ChemistryPhysics_wrangled, aes(x = depth, y = temperature_C, color = lakename))+
  geom_point(alpha = 0.5)+
  geom_smooth(method = lm, se = FALSE)+
  ylim(0,35)+
  labs(title = "Temperature by Depth", x = "Depth (m)", y = "Temperature (°C)", color = "Lake Name")
